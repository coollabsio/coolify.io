<script>
  ...
  async function fetchAllContributors() {
    ...
    return Array.from(all.values()).sort((a, b) => b.contributions - a.contributions);
  }
...
<div class="text-white max-w-6xl mx-auto py-4 px-4">
  ...
  {:else}
    ...
    <div class="flex flex-wrap justify-center gap-4 mb-10">
      <div class="bg-coolgray-300/60 text-sm px-4 py-2 rounded-full">
    ...
    <!-- Top Contributors -->
    <h2 class="text-2xl font-semibold mt-20 mb-2 text-center">Top Contributors</h2>
    <p class="text-neutral-400 text-center mb-10">Taking the lead in contributions</p>
    
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-8">
      {#each topContributors as c}
        <a href={c.html_url} target="_blank" class="bg-coolgray-200/70 rounded-lg p-6 flex flex-col items-center justify-center hover:bg-coolgray-300 transition h-56 text-center">
          <img src={c.avatar_url} alt={c.login} class="w-20 h-20 rounded-full mb-3" />
          <div class="font-normal mt-1 text-lg">{c.login}</div>
          <div class="text-md mt-1 text-neutral-400">{c.contributions} contributions</div>
        </a>
      {/each}
    </div>

    <!-- All Contributors -->
    <h2 class="text-xl font-semibold mt-24 mb-2 text-center">All Contributors</h2>
    <p class="text-neutral-400 text-center mb-10">Everyone improving Coolify</p>
    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-6">
      {#each contributors as c}
        <a href={c.html_url} target="_blank" class="bg-coolgray-200/70 rounded-lg p-4 flex flex-col items-center hover:bg-coolgray-300 transition text-center">
          ...
          <div class="text-xs mt-1 text-neutral-400">{c.contributions} contributions</div>
        </a>
      {/each}
    </div>
    ...
    <div class="mt-24 text-center">
      <h3 class="text-2xl font-semibold mb-4">Start Contributing to Coolify</h3>
      <p class="mb-6 text-neutral-300">Coolify is made better by every contribution. We welcome your help!</p>
